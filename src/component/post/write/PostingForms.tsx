...
import { MDXFile } from '../../../app/post/model/getMdxFileMapper'
                    ...
                    ({ isOpen, close }) =>
                      isOpen && (
                        <SubmitModal
                          ...
                          onClose={(inputState) => close({} as InputState)}
                          onConfirm={(inputState) => close(inputState)}
                        />
                      )
                  )

                  // 모달 종료버튼 누른경우 비어있음.
                  if (Object.keys(modalInput).length === 0) return
